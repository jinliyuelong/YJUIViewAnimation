//
//  MasoryAnimationViewController.m
//  YJUIViewAnimation
//
...
#import "MasoryAnimationViewController.h"
#import "Masonry.h"
#import "YJAnimateButton.h"
static NSString *tableId = @"tableCellid";
@interface MasoryAnimationViewController ()<UITableViewDelegate,UITableViewDataSource>
//约束
...
@property (nonatomic, assign)  CGFloat  loginWidthConstraint;//登录的宽度
@property (nonatomic, assign)  CGFloat  loginHeightConstraint;//登录的高度
...
@implementation MasoryAnimationViewController

// MARK:  - life period
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"Maasory动画约束";
    ...
    self.loginHeightConstraint = 46;
    self.loginWidthConstraint = [UIScreen mainScreen].bounds.size.width - 50;
...
- (void)viewWillLayoutSubviews{
    ...
    [_loginBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self.view);
        make.width.mas_equalTo(self.loginWidthConstraint);
        make.height.mas_equalTo(self.loginHeightConstraint);
...
- (void)actionToSignIn:(UIButton *)sender {
    ...
    if (_isAnimating) {
        [self.loginBtn start];
         self.loginWidthConstraint = self.loginHeightConstraint;
